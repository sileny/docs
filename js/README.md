...
此处，我们以所需的缓存名称 `CACHE_NAME` 调用 `caches.open()`，之后再调用 `cache.addAll()` 并传入文件数组。这是一个 `promise` 链（`caches.open()` 和 `cache.addAll()`）。`event.waitUntil()` 方法带有 `promise` 参数并使用它来判断安装所花费的时间，以及安装是否成功。
